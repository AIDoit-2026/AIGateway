...
async function handleServerCrash(code: number | null) {
  ...
  const result = await dialog.showMessageBox({
    type: 'error',
    title: 'AIGateway backend stopped',
    message: `The local AIGateway backend exited unexpectedly${typeof code === 'number' ? ` (code ${code})` : ''}.`,
...
async function restartBackend() {
  ...
  try {
    ...
    await dialog.showMessageBox({
      ...
      message: 'AIGateway could not restart the local backend.',
...
function setupAutoUpdater() {
  ...
  autoUpdater.on('update-available', async (info) => {
    const result = await dialog.showMessageBox({
      ...
      message: `AIGateway ${info.version} is available.`,
  ...
  autoUpdater.on('update-downloaded', async () => {
    const result = await dialog.showMessageBox({
      type: 'info',
      title: 'Update ready',
      message: 'The new AIGateway desktop update is ready to install.',
  ...
  app.whenReady()
    .then(async () => {
      try {
        ...
        log.error('Failed to boot AIGateway desktop', error);
        const result = await dialog.showMessageBox({
          type: 'error',
          title: 'AIGateway failed to start',
          message: 'The desktop shell could not start the local AIGateway service.',
          detail: error instanceof Error ? error.message : String(error),
          buttons: ['Retry', 'Quit'],
          defaultId: 0,
          cancelId: 1,
